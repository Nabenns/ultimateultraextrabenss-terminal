...
/** Resolves a role to a model override (or null to use the worker default). */
export type ModelResolver = (role: string) => string | null;
...
export class Orchestrator {
  ...
  constructor(
    ...
    private readonly modelFor: ModelResolver = () => null,
  ) {}
  ...
  async ensureSession(role: string): Promise<{ sessionID: string; deliver: (text: string) => Promise<void> }> {
    ...
    return { sessionID, deliver: (text: string) => client.promptAsync(sessionID, text, this.agentFor(role), this.modelFor(role)) };
  }
  ...
  async dispatch(assignments: Assignment[]): Promise<Job[]> {
    ...
    for (const a of assignments) {
      ...
      await client.promptAsync(sessionID, a.task, this.agentFor(a.role), this.modelFor(a.role));
